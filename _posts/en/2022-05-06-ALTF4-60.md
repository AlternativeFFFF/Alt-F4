...
**Editor's note:** After publishing, FuryoftheStars on the Factorio Forums reminded that machines in game use max power while active, while the article assumed that they use only operating power (which is max power - min power). Therefore, to match the game, for each machine, the min power value needs to be added to the written value for P which increases it by approximately 3%. Luckily, this does not significantly impact the article's conclusions. Thanks for reaching out!
