...
It's [worth repeating](https://alt-f4.blog/ALTF4-21/) that Factorio has an absolutely amazing modding community. However, you might not realize just how much effort some modders need to go through to create their masterpieces. Especially when they start running into engine limitations. The Factorio developers do [try and expand](https://forums.factorio.com/viewforum.php?f=28) the engine to give modders more flexibility, but it's impossible (at least in Factorio) to allow modders to do _everything they want_. So when modders reach some limitation in what Factorio allows them to do then they need to get creative to achieve their vision. What they come up with is usually then interesting and crazy ~~hacks~~ workarounds which the player often don't even notice. I always find these workarounds fascinating, so I reached out to a few modders and asked them about their favourite stories in how they bent Factorio to their will.
...
The vehicles would need to go from A to B, but they can't always go in a straight line. They need to find a path around obstacles. Writing a pathfinding algorithm is not that hard, but for Factorio modding the problem for a pathfinder is giving the algorithm an accurate representation of the map. Querying the map state in a way that would be useful would be unrealistically slow, and access to Factorio's C++ pathfinder wasn't accessible (at that time) so I needed something a bit more... creative.  The game does have a built in "unit" type that is used for biters, and they can be told to go to a destination and will use the built in pathfinder to do that. The problem is that units are not vehicles, they can't hold passengers, they don't have an inventory, and they can't use ammo.
